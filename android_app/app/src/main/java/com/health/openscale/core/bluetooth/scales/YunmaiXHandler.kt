 ...
 *
 * Full manufacturer AD structure (type 0xFF, 16 bytes; reverse-engineered from an
 * nRF Connect capture of a YMBS-M268, and confirmed against a live measurement
 * captured through this handler, 2026-07-08):
 *
 ...
 *
 * Android splits the first two bytes off as the manufacturer id (little-endian), so the
 * SparseArray payload is 14 bytes and all offsets shift by -2. Because the "manufacturer
 * id" is really part of the reversed MAC, it differs per unit and cannot be used for
 * matching; instead we verify the MAC echo, the 0B 74 17 signature and the checksum.
 */
internal object YunmaiXAdv {
    ...
    /**
     * Parse one manufacturer-data entry as split by Android.
     *
     * @param manufacturerId SparseArray key (first two AD bytes, little-endian)
     * @param payload        SparseArray value (remaining 14 bytes)
     * @param address        device MAC for the echo check; skipped if null/unparseable
     */
