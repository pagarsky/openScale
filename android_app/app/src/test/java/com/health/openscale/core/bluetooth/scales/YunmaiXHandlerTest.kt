...
class YunmaiXHandlerTest {
    ...
    // Final frame from a real measurement captured end-to-end through this handler
    // (2026-07-08). byte[9] here is 0x10 (vs 0x04 in the nRF fixtures above), guarding
    // the per-session-varying byte that must NOT be treated as a constant.
    private val FRAME_FINAL_LIVE = "8B0168ED0B741710031B8F020782" // state 03, 70.55 kg, 519 Ω
    ...
    @Test
    fun `parses live-captured final frame with differing per-session byte`() {
        val f = YunmaiXAdv.parse(MFR_ID, bytes(FRAME_FINAL_LIVE), ADDRESS)!!
        assertThat(f.state).isEqualTo(YunmaiXAdv.STATE_FINAL)
        assertThat(f.weightKg).isWithin(1e-3f).of(70.55f)
        assertThat(f.impedanceOhm).isEqualTo(519)
    }
